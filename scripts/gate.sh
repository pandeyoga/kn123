...
###############################################################################
# KUNCI TUNGGAL — dua gate TIDAK BOLEH berjalan bersamaan.
#
# Terukur 2026-08-23 (dan mahal): satu perintah latar belakang yang diulang oleh
# alat pemanggil menyalakan `gate.sh --full` DUA KALI. Keduanya menulis ke log
# yang sama dan — karena gate memulai dengan MENGHAPUS + menyemai ulang data uji
# — keduanya saling menarik lantai. Hasilnya **30+ gate merah palsu**: sidik jari
# anti-residu terekam saat basis data sedang KOSONG ("sales_orders: 0 -> 11"),
# saldo stok bergeser, dan dokumen demo hilang di tengah POC lain. Nol dari
# kegagalan itu adalah bug kode; membacanya sebagai bug akan menghabiskan satu
# sesi penuh.
#
# Karena itu: gate kedua BERHENTI BERISIK di sini, dengan menyebut PID yang
# sedang berjalan. Lebih baik menolak jalan daripada menghasilkan receipt yang
# tidak bisa dipercaya siapa pun.
###############################################################################
GATE_LOCK="${KN_GATE_LOCK:-/tmp/kn_gate.lock}"
if [ -e "$GATE_LOCK" ]; then
  OLD_PID="$(cat "$GATE_LOCK" 2>/dev/null || echo '')"
  if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
    echo "FATAL: gate lain SEDANG BERJALAN (PID $OLD_PID)."
    echo "       Dua gate bersamaan saling menghapus data uji → receipt palsu."
    echo "       Tunggu sampai selesai, atau hentikan: kill $OLD_PID"
    exit 3
  fi
  echo "(kunci gate basi dari PID ${OLD_PID:-?} dibuang — prosesnya sudah tidak ada)"
  rm -f "$GATE_LOCK"
fi
echo $$ > "$GATE_LOCK"
...
run_gate () {  # $1=label  $2..=command
  ...
  # PELACAK RESIDU (opsional, `KN_GATE_PROBE=1`): merekam jumlah dokumen + hasil
  # audit relasi dokumen SESUDAH setiap langkah, supaya "gate mana yang mengotori
  # data" terjawab dengan bukti, bukan tebakan. Lihat scripts/gate_probe.py.
  if [ "${KN_GATE_PROBE:-}" = "1" ]; then
    python scripts/gate_probe.py --after "$label" 2>&1 | tail -1
  fi
}
...
STATIK=(
  ...
  # AUDIT 2026-09 T-03 — INV-PERF-01. `to_list(n>20000)` memuat seluruh koleksi ke memori
  # (63 lokasi, beberapa lalu menyaring di Python). Ratchet: angka hanya boleh turun;
  # kejadian baru wajib alasan tertulis di ALLOWLIST skrip.
  "guard:to_list_bound SELF-TEST (bukti-merah dua arah, 11 kasus)|python scripts/guardrails/verify_to_list_bound.py --self-test"
  "guard:to_list_bound (INV-PERF-01, to_list(n>20000) dilarang)|python scripts/guardrails/verify_to_list_bound.py"
  # AUDIT 2026-09 T-07 — INV-DOC-01. CODEBASE_MAP.md adalah bacaan TIER 0 tiap sesi;
  # versi tulisan tangan menyimpang 10x (25 vs 123 router). Kini dihasilkan
  ...
  # INV-REF-04 — `link()` menulis relasi DUA ARAH, jadi siapa pun yang MENGHAPUS
  # dokumen wajib menyapu tautan baliknya. Kewajiban itu tak pernah ditulis, dan
  # terukur 2026-08-21 ada LIMA titik penghapusan PRODUKSI tanpa sapuan: amandemen
  # PO menghapus inbound `wms_tasks` (doc_type `grn`) yang sudah bertaut ke PO-nya ·
  # hapus retur beli draf · hapus kontrak supplier · 2× rollback transfer saat SO
  # gagal. Gejalanya muncul lewat residu POC E-7 (`audit_doc_refs --strict` MERAH
  # "Tautan menggantung: 2 temuan") — dan hanya pada gate BERIKUTNYA, karena audit
  # data berjalan SEBELUM POC. Penjaga statik ini menutup SUMBERNYA.
  "guard:ref_unlink SELF-TEST (bukti-merah dua arah, 8 kasus)|python scripts/guardrails/verify_ref_unlink.py --self-test"
  "guard:ref_unlink (INV-REF-04, hapus dokumen wajib sapu tautan balik)|python scripts/guardrails/verify_ref_unlink.py"
  # FASE N — INV-NOTIF-02. Dua kelas cacat alamat notifikasi, keduanya SENYAP:
  #  (K1) peristiwa ber-PEMILIK JELAS disiarkan ke `recipient_role="all"` — terukur
  #       11 dari 35 notifikasi data demo (low_stock 9 · order_approval 1 ·
  #       internal_request_decided 1), sehingga Finance membuka kotaknya dan menemukan
  #       SEMBILAN pesan stok kain yang tak bisa ia tindaklanjuti. Kotak yang isinya
  #       bukan urusan kita adalah kotak yang berhenti dibaca — dan sesudah itu
  #       peringatan pertama yang sungguh penting pun ikut tak terbaca.
  #  (K2) `recipient_role` ditulis BERSAMA `recipient_user`. Ini yang paling menipu:
  #       penyaring pembaca memakai OR, jadi `recipient_role="sales"` +
  #       `recipient_user=<pemegang akun>` BUKAN menyempitkan alamat melainkan
  #       menyiarkan ke SELURUH sales. Penulisnya yakin sedang mempersempit. Terukur
  #       di 5 titik PRODUKSI (job piutang · barang pendingan datang · AR overdue ·
  #       2× persetujuan harga: "Aturan harga ANDA diganti" dikirim ke semua sales).
  # Gate memeriksa KODE (ast) DAN DATA (dokumen hidup ber-alamat "all"), karena kode
  # bisa sudah benar sementara dokumen warisan masih menyiarkan (utang D5).
  "guard:notif_audience SELF-TEST (bukti-merah dua arah, 14 kasus)|python scripts/guardrails/verify_notification_audience.py --self-test"
  "guard:notif_audience (INV-NOTIF-02, alamat notifikasi dari WEWENANG)|python scripts/guardrails/verify_notification_audience.py"
...
# TETAPI TIDAK BOLEH menumpang bersama `seed_realistic`.
#
# RACE NYATA (terukur 2026-08-21 — 1 dari 7 jalan `--full`, jadi ia memang jarang
# dan justru itu bahayanya): `seed_realistic` MENGHAPUS ±60 koleksi lalu mengisinya
# ulang selama ~15 s. Sebagian gate di kolam statik menyentuh Mongo/API SUNGGUHAN
# (`audit_config_wiring` memakai `MongoClient`; `audit_entity_isolation --self-test`
# MENULIS lalu memulihkan fixture dua entitas). Kalau salah satunya jatuh tepat di
# JENDELA KOSONG itu, ia membaca/memulihkan keadaan kosong sehingga sesudah seed
# selesai koleksinya TETAP kosong.
#
# Gejalanya sudah pernah terlihat dan sangat menyesatkan: tepat sesudah
# "seed_realistic PASS", `verify_data_integrity` melapor `suppliers`,
# `payment_terms`, `approval_rules`, `purchase_requisitions` **KOSONG**, lalu 15
# gate berikutnya memerah beruntun (cakupan relasi dokumen jatuh ke 0–17%, lima POC
# FASE G mati) — semuanya BUKAN bug, semuanya sesat. Satu jalan berikutnya hijau
# lagi tanpa ada kode yang berubah, dan gate yang "kadang merah tanpa sebab" adalah
# gate yang akhirnya diabaikan orang.
#
# Pengecualian dengan alasan yang SAMA sudah ada di berkas ini untuk
# `numeric_bounds` ("WAJIB berurutan & SEBELUM seed") — kolam statiknya saja yang
# belum. Biaya menundanya ~nol: kolam statik ±40 s, blok runtime sesudah seed ±250 s.
STATIK_PID=""
STATIK_STARTED=0
start_statik () {
  [ "$STATIK_STARTED" -eq 1 ] && return 0
  STATIK_STARTED=1
  if [ "$PARALLEL" -eq 1 ]; then
    echo -e "${YEL}  Gate STATIK dijalankan di latar belakang (${#STATIK[@]} gate · ${JOBS} pekerja).${RST}"
    statik_pool & STATIK_PID=$!
  else
    # Berurutan: hasilnya dicetak langsung oleh `run_gate`, jadi `STATIK_PID`
    # DIBIARKAN kosong supaya `collect_statik` (pembaca berkas kolam) tidak jalan
    # dan tidak melaporkan 34 gate "FAIL" karena berkasnya memang tak pernah ada.
    for spec in "${STATIK[@]}"; do run_gate "${spec%%|*}" "${spec#*|}"; done
  fi
}
# Tanpa Mongo atau mode --quick: tidak ada seed yang bisa ditabrak → mulai sekarang.
if [ "$MODE" = "quick" ] || [ -z "${MONGO_URL:-}" ]; then start_statik; fi
...
if [ -n "${MONGO_URL:-}" ]; then
  ...
  # Seed SUDAH selesai → aman menyalakan kolam statik (lihat catatan RACE di atas).
  start_statik
...
if [ $AUTH_READY -eq 1 ]; then
  ...
  # INV-AGING-01 (2026-08-25) — "FIELD DITEBAK". `AGING_META` menyebut dari mana umur
  # tunggu dihitung; baris `special_order` menyebut `submitted_at`/`approval_requested_at`
  # yang NOL jalur tulis, jadi umurnya diam-diam jatuh ke `created_at` DAN pengurutan
  # papan memakai field null (yang tertua bisa terpotong). Mongo tak protes untuk field
  # yang tak ada — hanya penjaga ini yang bisa menangkap seluruh KELASNYA. Saat pertama
  # dijalankan ia langsung menemukan 3 field tebakan lain (`submitted_for_approval_at`,
  # dua `*_warehouse_name` yang ternyata field TURUNAN saat dibaca).
  run_gate "guard:aging_fields SELF-TEST (bukti-merah field umur ditebak)" "python scripts/guardrails/verify_aging_fields.py --self-test"
  run_gate "guard:aging_fields (INV-AGING-01, field umur tunggu wajib nyata)" "python scripts/guardrails/verify_aging_fields.py"
  # INV-HIST-01 (2026-06) — `status_history[]` hanya boleh punya SATU bentuk. Dulu
  # `special_orders` menulis `{"timestamp","user"}` sementara `inventory_lots` menulis
  # `{"at","actor"}`: pembaca lintas koleksi mendapat `None` tanpa galat lalu jatuh ke
  # `created_at` — kelas cacat B1 lewat pintu belakang.
  run_gate "guard:status_history SELF-TEST (bukti-merah bentuk riwayat ke-dua)" "python scripts/guardrails/verify_status_history.py --self-test"
  run_gate "guard:status_history (INV-HIST-01, satu bentuk riwayat status)" "python scripts/guardrails/verify_status_history.py"
  # D2 (2026-08-25) — POC PERILAKU untuk Papan PO Custom. Papan ini lahir sesi
  # 2026-08-24 dan hijau di seluruh gate, lalu audit mandiri menemukan 6 cacat yang
  # tak satu pun tertangkap: buktinya hanya penjaga statik/HTTP terhadap data demo
  # yang KEBETULAN ada. POC ini MEMBUAT dokumen berumur (2 · 15 · 60 hari), mengukur
  # papannya, lalu membersihkannya — bukti-merah untuk B1/B2/B3/B4 + nol residu.
  run_gate "POC Papan PO Custom (umur tunggu bukan tebakan · yang tertua ikut · jujur saat dipotong · nol residu)" "python backend/test_core_papan_po_custom_poc.py"
  # SESI 2026-06 — true-up persediaan tak terkunci kalender (INV-GL-DRIFT) · papan
  # antrean mahal juga di Dasbor Manajer · lencana umur tunggu dipakai ulang untuk
  # kontrabon bersengketa & retur antar-PT · satu bentuk `status_history` (INV-HIST-01).
  run_gate "POC Sesi 2026-06 (true-up persediaan · papan manajer · umur tunggu antrean lain · satu bentuk riwayat)" "python backend/test_core_sesi_2026_06_poc.py"
  ...
  # ── P-0 (prasyarat FASE P) — ASAL DOKUMEN PO: rantai PO→PR→SO & kolom Nama Sales.
  # PO lahir dari EMPAT pintu (realisasi PR · award RFQ · PO manual/call-off · blanket).
  # Tiga cara fitur ini membusuk, semuanya SENYAP: pintu ke-5 lahir tanpa asal dokumen
  # (papan benar untuk SEBAGIAN dokumen saja) · daftar field disalin lalu menyimpang
  # (papan tak bisa bedakan "tidak ada sales-nya" dari "field-nya tidak ada") · nama
  # sales diturunkan dari PEMBUAT PO (papan berbohong dengan tenang tentang siapa yang
  # menjual). INV-ORIG-01 menjaga ketiganya + menuntut refs PO↔PR dua arah, dan
  # SENGAJA tidak menuduh 14 PO lama yang `pr_id`-nya kosong (keputusan pemilik
  # 2026-08-21: dokumen lama dibiarkan, tanpa backfill).
  run_gate "guard:doc_origin SELF-TEST (bukti-merah asal dokumen PO, 26 kasus dua arah)" "python scripts/guardrails/verify_doc_origin.py --self-test"
  run_gate "guard:doc_origin (INV-ORIG-01, satu definisi asal PO · sales dirunut bukan diketik · refs dua arah)" "python scripts/guardrails/verify_doc_origin.py"
  # POC P-0 — bukti PERILAKU (bukan pola kode): tiga pintu lahirnya PO menulis asal
  # dokumen yang sama, Nama Sales dirunut dari pesanan walau yang menekan tombol adalah
  # admin, PO rutin tetap kosong-jujur, refs dua arah, dan data demo punya rantainya.
  run_gate "POC P-0 (asal dokumen PO: PO→PR→SO · Nama Sales dirunut · PO rutin kosong-jujur · nol residu)" "python backend/test_core_p0_poc.py"
  # ── FASE P — PAPAN PO PER LINI (kertas kerja MD). Lima kegagalan senyap yang
  # dijaga `INV-STAGE-01`: urutan tahap menunjuk kode yang TIDAK ADA di master
  # (papan menampilkan label hasil menebak, dan master yang diubah pemilik tidak
  # berpengaruh) · tahap `inspect` bisa ditandai tangan (papan mengaku sudah
  # diinspeksi tanpa dokumen inspeksi) · pintu ke-5 lahirnya PO tanpa field papan ·
  # tanda tahap tanpa siapa/kapan · tanda tahap di luar urutan lini dokumen.
  run_gate "guard:po_board SELF-TEST (bukti-merah papan PO, 22 kasus dua arah)" "python scripts/guardrails/verify_po_board.py --self-test"
  run_gate "guard:po_board (INV-STAGE-01, tahap dari master · inspect turunan · tanda tahap ber-jejak)" "python scripts/guardrails/verify_po_board.py"
  # POC FASE P — bukti PERILAKU: urutan tahap DIBACA dari master (POC mengubah master
  # lalu mengukur papan), Nama Sales dirunut PO→PR→SO, tahap inspeksi ditolak 409,
  # tanggal masuk & qty terima berubah SENDIRI sesudah penerimaan gudang yang nyata,
  # pagar lini & badan usaha keras di server, kartu ringkasan == hitung-ulang mandiri.
  run_gate "POC FASE P (papan PO per lini: tahap dari master · sales dirunut · inspect turunan · terima dihitung · nol residu)" "python backend/test_core_po_board_poc.py"
  # ── FASE D (rencana MD-ERP) — PERMINTAAN DESAIN `<ENT>/DSR-#####` + peran ke-7
  # `designer`. JANGAN keliru dengan "POC FASE D (makloon rantai proses)" di blok POC
  # di bawah: itu fase lain dari penomoran lama (`test_fase_d_makloon_poc.py`).
  # Kenapa POC ini ada di blok yang butuh backend hidup: seluruh buktinya adalah
  # PERILAKU (mesin keadaan, pagar izin, angka rapor vs hitung-ulang), bukan pola kode.
  # Ia juga menjalankan `check_nav_map` + `INV-APPR-01` + `INV-HOME-01` sungguhan dan
  # menuntut daftar peran DIBACA dari registry — kelas cacat yang pernah membuat
  # `check_nav_map` CRASH `KeyError: 'designer'` begitu peran ke-7 lahir.
  run_gate "POC FASE D · PERMINTAAN DESAIN (DSR) (tugas→serah→revisi ber-alasan→ACC · rapor = hitung-ulang · peran ke-7 sempit tapi jujur)" "python backend/test_core_design_request_poc.py"
  # ── FASE S — SAMPLING SUPPLIER (labdip · handfeel · proofing, BOLEH LEBIH DARI SATU).
  # `INV-SAMPLE-01` menjaga tujuh kegagalan senyap yang dibuka fase ini: jenis terpakai
  # dihapus/dinonaktifkan (lencana tanpa nama) · DUA SUMBER jenis (`sample_type` lama
  # menempel di samping `sample_types[]`) · round tanpa/salah `type_code` (skor warna
  # dibandingkan dengan skor rasa kain seolah setara) · `measurement_fields` menyebut
  # field yang tak punya kamus (form meminta kolom tanpa label & tanpa batas) · hasil
  # ukur round menyimpang dari jenisnya · jenis ber-`requires_design` dipakai tanpa
  # kode desain (aturan yang dulu dijaga `if` di kode) · urutan pelaksanaan
  # jadi→kirim + tujuan wajib, termasuk penjagaan STATIK bahwa `finish`/`deliver`
  # tetap PELAKSANAAN (izin `rnd.submit`) — begitu izinnya naik jadi keputusan, gate
  # menuntut antreannya didaftarkan. Plus anti-regresi D7: nomor `<ENT>/SMP-#####`
  # wajib berpola & UNIK (seeder yang menomori sendiri pernah membuat 5 nomor kembar).
  run_gate "guard:sample_types SELF-TEST (bukti-merah jenis sampling, 37 kasus dua arah)" "python scripts/guardrails/verify_sample_types.py --self-test"
  run_gate "guard:sample_types (INV-SAMPLE-01, jenis dari master · satu sumber · hasil ukur dinamis · jadi→kirim)" "python scripts/guardrails/verify_sample_types.py"
  # POC FASE S — bukti PERILAKU (bukan pola kode): satu permintaan menempuh DUA jenis
  # dengan dua rangkaian round yang riwayatnya tidak tercampur, hasil ukur wajib
  # DIBACA dari master (POC mengubah master lalu mengukur penolakan server), kuota
  # round dihitung per (supplier × jenis), tautan SO muncul di jejak dokumen, dan
  # "jadi → kirim" menolak urutan yang mustahil. Nol residu (stok & dokumen).
  run_gate "POC FASE S (sampling: dua jenis paralel · ukur dari master · jadi→kirim · nol residu)" "python backend/test_core_sampling_poc.py"
  # ── FASE I — INSPEKSI & QC SEBAGAI DOKUMEN (SPK · tahanan warna · grade).
  # `INV-QC-01..03` (lapisan `inspection` di verify_data_integrity) menjaga inti fase
  # ini: dokumen inspeksi TIDAK boleh menjadi pintu ke-3 untuk grade. POC di bawah
  # membuktikan PERILAKUNYA, bukan pola kode: SPK lahir OTOMATIS & idempotent dari
  # penerimaan PO · poin/grade dihitung mesin 4-point lama (grade_history bertambah
  # TEPAT SATU) · warna beda menahan roll sampai MANAJER melepasnya ber-alasan
  # (putaway dijawab 400 ber-pesan menuntun, gudang 403) · retur pelanggan punya SPK
  # + tonggak perjalanan tanpa menduplikasi hasil returnya · `tolak`/`reopen` wajib
  # ber-alasan yang tersimpan DI DOKUMEN · IDOR lintas PT · dan invarian INV-QC
  # DIBUKTIKAN BISA MEMERAH lalu hijau lagi. Nol residu (stok · jurnal · dokumen).
  run_gate "POC FASE I (inspeksi & QC: SPK otomatis · grade dari mesin lama · tahanan warna dilepas manajer · nol residu)" "python backend/test_core_inspeksi_poc.py"
  # FASE N — alamat notifikasi. POC ini membuktikan PERILAKUnya, bukan hanya polanya:
  # kotak Finance benar-benar bersih dari pesan stok, Finance benar-benar masuk daftar
  # penerima piutang, PO custom yang diajukan benar-benar memberi tahu pemegang
  # keputusan, dedupe berjalan PER ORANG (job dua kali → tiap orang tetap satu pesan),
  # dan mencabut izin dari layar Matriks Izin benar-benar mengubah daftar penerima
  # tanpa satu baris kode diubah. Nol residu (koleksi notifikasi dipulihkan PERSIS).
  run_gate "POC FASE N (notifikasi beralamat: izin & divisi · dedupe per orang · nol siaran 'all')" "python backend/test_core_notifikasi_alamat_poc.py"

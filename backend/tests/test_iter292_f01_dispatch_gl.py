...
def test_f01_dispatch_posts_revenue_and_cogs(hdr):
    ...
    for stype in ("sales_order", "sales_cogs"):
        je = list(db.journal_entries.find({"source_type": stype, "source_id": ORDER},
                                          {"_id": 0, "id": 1, "status": 1, "lines": 1}))
        assert je, f"no journal entry with source_type={stype} for {ORDER} right after dispatch"
        assert any(j.get("status") == "posted" for j in je), f"{stype} JE not posted: {je}"
        print(stype, "JE ok:", [j["id"] for j in je])

    # API surface must expose it too
    r = requests.get(f"{BASE}/gl/journal", headers=hdr, params={"source": "sales_order"}, timeout=60)
    assert r.status_code == 200, r.text[:300]
    data = r.json()
    items = data if isinstance(data, list) else data.get("items", data.get("entries", []))
    assert any(i.get("source_id") == ORDER for i in items), \
        f"GET /gl/journal?source=sales_order does not include {ORDER} (got {len(items)} rows)"

...
| **INV-HOME-01** | Angka KPI beranda WAJIB sama dengan kenyataan: (A) `approvals_pending` == `approvals.total` di payload yang sama; (B) tiap baris == hitung-ulang MANDIRI dari MongoDB oleh gate (opini kedua); (C) bila ADA dokumen menunggu keputusan, KPI tidak boleh 0; (D) tiap baris menunjuk `view` yang ADA di `AppViewRouter.jsx`; (E) tiap koleksi antrean HARUS ada di DB; (F) tiap antrean punya pasangan hitung-ulang di gate | **KN-F3-KPI-LIES (P1 senyap)**: KPI "Persetujuan Menunggu" di Control Tower & Beranda Manajer memakai `approval_service.get_pending_approvals_count()` yang menghitung koleksi `approval_requests` — koleksi yang **tak pernah diisi siapa pun** (`create_approval_request()` nol pemanggil). Angkanya **selalu 0** sementara daftar rincian di layar yang sama berbunyi 6 dan kenyataan 17 dokumen menunggu (1 SO · 3 PO · 2 harga khusus · 1 PR · 1 retur jual · 2 retur beli · 1 amandemen · 1 opname · 1 spesifikasi · 3 sample · 1 pesanan khusus). Tidak ada error, tidak ada uji gagal: orang yang pekerjaannya MENYETUJUI membaca "0" lalu pulang. Termasuk kelas "salah nama koleksi" (`amendments` vs `doc_amendments` → satu antrean hilang tanpa pesan) | `scripts/guardrails/verify_home_kpi.py` (+`--self-test` bukti-merah) · `backend/test_core_role_access_poc.py` G3/G6 | RUNTIME | `backend/services/approval_backlog_service.py` (`QUEUES` = SSOT definisi antrean) | · **(G)** daftar "paling lama menunggu" hanya dari antrean ber-hitungan >0 & layar nyata · **(H)** PAPAN PO CUSTOM: `special_orders_waiting` **WAJIB ADA untuk beranda `admin`** (D1 2026-08-25 — dulu invarian H seluruhnya DILEWATI bila kuncinya dihapus: pagar yang bisa dimatikan dengan menghapus datanya bukan pagar), jumlahnya == baris antrean `special_order`, dan bila `count > len(rows)` payload WAJIB mengirim `shown`/`hidden`/`truncated` (B2 — judul "(12)" di atas 10 baris tanpa penanda = angka yang tak cocok dengan daftar di layar yang sama)
| **INV-AGING-01** | FIELD UMUR TUNGGU TIDAK BOLEH DITEBAK: setiap kandidat field di `approval_backlog_service.AGING_META[*]` (`since`/`number`/`title`, selain `created_at`) WAJIB terbukti nyata — muncul di ≥1 dokumen koleksi antreannya ATAU disebut sebagai kunci tulis di kode backend | **B1 (2026-08-24, P1 senyap)**: baris `special_order` membaca umur dari `submitted_at`/`approval_requested_at` yang NOL jalur tulis → seluruh dokumen `null`, umur diam-diam jatuh ke `created_at` (dokumen lama-draf dilaporkan jauh lebih tua) DAN pengurutan papan memakai field null sehingga `limit(10)` memotong dokumen yang PALING TUA. Mongo tidak protes untuk field yang tak ada — tak ada galat, tak ada uji gagal. Saat penjaga ini pertama dijalankan ia menemukan 3 field tebakan LAIN: `sales_orders.submitted_for_approval_at`, `warehouse_transfers.dest_warehouse_name`/`source_warehouse_name` (dua terakhir field TURUNAN saat dibaca, bukan tersimpan) | `scripts/guardrails/verify_aging_fields.py` (+`--self-test` bukti-merah) · `backend/test_core_papan_po_custom_poc.py` P1/P2 | STATIK+DATA | `backend/services/approval_backlog_service.py` (`AGING_META`) |
...
| **INV-REF-04** | **MENGHAPUS DOKUMEN WAJIB MENYAPU TAUTAN BALIKNYA.** `doc_refs_service.link()` menulis relasi DUA ARAH, jadi tiap `db.<koleksi>.delete_one/delete_many` pada koleksi ber-`refs` (34 koleksi, dibaca dari registry `DOC_TYPES`) WAJIB berada di fungsi yang juga memanggil `unlink_all()`/`safe_unlink_all()`. Nama koleksi wajib bisa dipastikan STATIK (literal/konstanta modul, termasuk lintas-modul `db[cs.COLL]`); yang dinamis harus terdaftar di `DYNAMIC_OK` beserta alasannya. POC/skrip uji dibebaskan dari aturan per-fungsi tetapi WAJIB memanggil `poc_stock_guard.sweep_ghost_refs()` di cleanup-nya — dan itu ditagih oleh lapisan RUNTIME di `gate_residue.py --check` | Kewajiban "sapu tautan balik" tak pernah ditulis di mana pun, jadi dokumen yang MASIH HIDUP memegang ref ke dokumen yang sudah tidak ada dan panel **"Referensi Dokumen"** dengan tenang menawarkan tautan yang menuju **404**. Terukur 2026-08-21: **5 titik PRODUKSI** (amandemen PO menghapus inbound `wms_tasks` = doc_type `grn` yang sudah bertaut ke PO-nya · hapus retur beli draf · hapus kontrak supplier · 2× rollback transfer saat SO gagal) dan **4 POC** (E-7 · FASE T · G-9 · G-6b). Gejala yang menghentikan sesi sebelumnya: `audit_doc_refs --strict` → *"Tautan menggantung: 2 temuan"* (`internal_requests KSC/PIN-00003 --fulfilled_by--> interco_transaction` ×2) yang juga MENJATUHKAN **POC P-0** — POC memerah karena kesalahan POC LAIN. Tak satu pun dari 237 invarian menangkapnya, dan `gate_residue` buta karena koleksinya tak dipantau. Lebih buruk: di `gate.sh` `audit_doc_refs` berjalan SEBELUM semua POC, jadi hantu yang ditinggalkan POC baru terlihat pada gate BERIKUTNYA — satu putaran terlambat, saat tak ada lagi yang menghubungkannya dengan penyebabnya | `scripts/guardrails/verify_ref_unlink.py` (+`--self-test` **8 kasus dua arah**, termasuk 2 anti-tuduh-palsu & 1 kasus "kata `unlink_all` cuma di komentar"). Bukti-merah pada kode SEBELUM perbaikan: **5 pelanggaran** di 4 berkas produksi. Lapis RUNTIME: `scripts/gate_residue.py --check` (tautan menggantung TIDAK pernah diabaikan, bahkan dengan `--ignore-trails`) · `scripts/audit_doc_refs.py --strict` §C | STATIK (`ast`, bukan regex — komentar & string tidak bisa mengelabui) + RUNTIME | `backend/services/doc_refs_service.py` (`unlink_all`/`safe_unlink_all`/`dangling_refs` = SSOT) · `backend/poc_stock_guard.py` (`sweep_ghost_refs` untuk POC) · alat diagnosis `scripts/ukur_residu_poc.py` |
| **INV-PERF-01** | `to_list(n)` dengan **n > 20000** DILARANG di kode produksi backend (memuat seluruh koleksi ke memori lalu menyaring di Python). Ratchet: angka hanya boleh TURUN; kejadian baru wajib alasan ≥20 huruf di `ALLOWLIST`; entri allowlist yang kodenya sudah bersih → MERAH juga | T-03 audit 2026-09: 63 lokasi `to_list(≥20000)`, `product_sales_velocity` memuat semua `inventory_movements` untuk memilih 2 jenis mutasi | `scripts/guardrails/verify_to_list_bound.py` | STATIK | `ALLOWLIST` di skrip (18 lokasi warisan ber-alasan "hutang T-03") |
| **INV-DOC-01** | `CODEBASE_MAP.md` (bacaan TIER 0) WAJIB dihasilkan `scripts/gen_codebase_map.py`; angka router/endpoint/service/koleksi di peta menyimpang >10% dari hitung ulang → MERAH; tanpa penanda generator (disunting tangan) → MERAH | T-07 audit 2026-09: peta v1.0 (28 Mei) 25 router/106 endpoint vs nyata 122/1.120; `hash_password` disebut SHA256 padahal bcrypt | `scripts/guardrails/verify_codebase_map.py` | STATIK | — (jalankan ulang generator) |
...
## INV-SAMPLE-01 — JENIS SAMPLING: MASTER vs DOKUMEN (FASE S · 2026-08-23)

Penjaga: `scripts/guardrails/verify_sample_types.py` (`--self-test` **37 kasus dua arah**).
Terdaftar di `scripts/gate.sh` (self-test + gate + POC FASE S).

FASE S memindahkan dua kelakuan dari KODE ke DATA, dan mengubah satu fakta tunggal
menjadi DAFTAR:
    ...
    sample_type (1 kata) → sample_types[] (daftar) + rounds[].type_code

Tujuh invarian yang menjaga kebebasan itu tidak berubah menjadi kebohongan senyap:

- **A — jenis yang MASIH dipakai dokumen tidak boleh hilang/nonaktif.** Penjaga
  MENGHITUNG dokumen pemakainya sebelum menuduh; `bulk_sample` yang memang
  dinonaktifkan & tak terpakai TIDAK dituduh (anti tuduhan palsu).
- **B — SATU SUMBER jenis.** Nol sisa `sample_type`, dan nol dokumen tanpa jenis
  (dokumen tanpa jenis tak muncul di penyaring mana pun → pekerjaannya tak terlihat,
  kelas cacat yang sama dengan FASE L).
- **C — tiap round punya `type_code`, dan jenisnya bagian `sample_types[]` dokumen.**
  Tanpa ini skor labdip (warna tepat) dibandingkan dengan skor handfeel (rasa tepat)
  seolah setara.
- **D — `measurement_fields` ⊆ kamus `domain_registry.SAMPLE_MEASUREMENTS`.** Field
  tanpa kamus = kolom tanpa label/satuan/batas: petugas mengisi angka yang tak pernah
  bisa dinilai siapa pun.
- **E — kunci `rounds[].measurements` == `measurement_fields` jenis round itu.** Dua
  pembebasan DISENGAJA: round yang masih TERBUKA, dan round historis/impor yang JUJUR
  menyatakan tanpa bukti (`proof_required: False` **dan** kosong seluruhnya — pola
  kejujuran yang sama dengan INV-RND-02). Yang mengisi SEBAGIAN tetap diperiksa:
  separuh angka lebih menyesatkan daripada tidak ada angka.
- **F — jenis ber-`requires_design` wajib merujuk desain NYATA.**
- **G — urutan & wewenang pelaksanaan.** Data: `delivered_at` tanpa `finished_at`
  (dikirim sebelum jadi), `delivered_at` tanpa `delivered_to`, tujuan di luar
  `sample_deliver_target`, tujuan tanpa tanggal kirim. Statik: `finish`/`deliver`
  WAJIB tetap memakai izin **pelaksana** (`rnd.submit`) dan alasan "SENGAJA BUKAN
  ANTREAN" wajib tertulis di `services/approval_backlog_service.py`.
  **Inilah yang menjaga keputusan pemilik "kirim sample tidak butuh persetujuan":**
  begitu izinnya naik ke `rnd.decide`, gate menuntut antreannya didaftarkan di
  `QUEUES` — kalau tidak, dokumen yang menunggu tak akan pernah muncul di KPI beranda.
- **D7 (anti-regresi) — nomor `md_samples` berpola `<ENT>/SMP-#####` dan UNIK.**
  Bukti-merah nyata: `scripts/seed_rnd_kpi_demo.py` dulu menomori sendiri dengan
  f-string `f"KSC/SMP-H{back}{designer[:2].upper()}"` → 20 dari 28 nomor menyimpang dan
  **5 pasang NOMOR DOKUMEN KEMBAR** (dua desainer ber-awalan huruf sama). Kini seeder
  memakai `core_utils.next_doc_number()` — satu-satunya penomor dokumen di repo ini.

**Kenapa alasan pembebasan `finish`/`deliver` TIDAK ditulis di
`verify_approval_queues.DOOR_EXEMPT`:** `DOOR_RE` hanya mengenali path ber-kata
`approve|reject|verify|decide`, sehingga entri untuk `/finish` & `/deliver` akan langsung
dianggap "klasifikasi basi" dan memerahkan INV-APPR-01 atas alasan yang salah. Alasannya
ditulis di tempat orang berikutnya mencari (daftar `QUEUES`) dan DIJAGA oleh aturan G di
atas — pembebasan yang diperiksa, bukan prosa yang tidak diperiksa.
